...
import {
  ...
  useCreateAthleteClusterMaster,
  useUpdateAthleteClusterMaster,
  useDeleteAthleteClusterMaster,
  useAthleteSubClusters,
  useCreateAthleteSubCluster,
...
export function AthleteClustersPage() {
  ...
  const createSubCluster = useCreateAthleteSubCluster();
  const updateSubCluster = useUpdateAthleteSubCluster();
  const deleteSubCluster = useDeleteAthleteSubCluster();
  ...
  const openClusterModal = (mode, item = null) => {
    ...
    setClusterForm(item ? {
      ...
      is_development_cluster: !!item.is_development_cluster,
              ...
              <button key={cluster.id} onClick={() => setSelectedClusterId(String(cluster.id))} className={`w-full text-left p-4 hover:bg-slate-50 transition-colors ${String(effectiveSelectedClusterId) === String(cluster.id) ? 'bg-red-50' : ''}`}>
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="text-xs text-slate-500">{cluster.code}</p>
                  </div>
                  <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${cluster.is_active ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-500'}`}>{cluster.is_active ? 'Aktif' : 'Nonaktif'}</span>
                </div>
                {cluster.is_development_cluster && <span className="mt-2 inline-flex items-center gap-1 text-xs font-medium text-green-700"><ShieldCheck className="w-3.5 h-3.5" /> Binaan KONI</span>}
                <div className="mt-3 flex gap-2">
                  <span onClick={(e) => { e.stopPropagation(); openClusterModal('edit', cluster); }} className="inline-flex items-center gap-1 px-2 py-1 text-xs text-blue-600 hover:bg-blue-50 rounded-lg"><Edit2 className="w-3.5 h-3.5" /> Edit</span>
                  <span onClick={(e) => { e.stopPropagation(); setDeleteTarget({ type: 'cluster', item: cluster }); }} className="inline-flex items-center gap-1 px-2 py-1 text-xs text-red-600 hover:bg-red-50 rounded-lg"><Trash2 className="w-3.5 h-3.5" /> Hapus</span>
                </div>
              </button>
            ))}
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-slate-100">
                {loadingSubClusters ? <tr><td colSpan={3} className="px-6 py-10 text-center"><Loader2 className="w-6 h-6 animate-spin text-slate-400 mx-auto" /></td></tr> : subClusters.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50">
                    <td className="px-6 py-4"><div className="flex items-center gap-3"><div className="w-8 h-8 bg-blue-100 rounded-lg flex items-center justify-center"><Layers className="w-4 h-4 text-blue-600" /></div><div><p className="font-medium text-slate-800">{item.name}</p><p className="text-xs text-slate-500">{item.code}</p></div></div></td>
                    <td className="px-6 py-4 text-center"><span className={`px-2 py-1 rounded-full text-xs font-medium ${item.is_active ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-500'}`}>{item.is_active ? 'Aktif' : 'Nonaktif'}</span></td>
...
function DeleteModal({ target, onClose, onConfirm, loading }) {
  ...
  return <Modal title="Hapus Data" onClose={onClose}><div className="p-6"><p className="text-sm text-slate-600">Hapus <strong>{target.item.name}</strong>? Data yang masih digunakan akan ditolak oleh sistem.</p><div className="flex justify-end gap-3 mt-6"><button onClick={onClose} className="px-4 py-2.5 border border-slate-200 rounded-xl text-slate-600 hover:bg-slate-50">Batal</button><button onClick={onConfirm} disabled={loading} className="px-4 py-2.5 bg-red-600 text-white rounded-xl font-semibold hover:bg-red-700 disabled:opacity-50">Hapus</button></div></div></Modal>;
}
...
function TextInput({ label, value, onChange, type = 'text', required = false }) {
  return <label className="block"><span className="block text-sm font-medium text-slate-700 mb-1">{label}</span><input type={type} value={value} onChange={(e) => onChange(e.target.value)} className="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-red-100 focus:border-red-500 outline-none" required={required} /></label>;
}

function TextArea({ label, value, onChange }) {
  return <label className="block"><span className="block text-sm font-medium text-slate-700 mb-1">{label}</span><textarea value={value} onChange={(e) => onChange(e.target.value)} rows={3} className="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-red-100 focus:border-red-500 outline-none" /></label>;
}

function CheckInput({ label, checked, onChange }) {
  return <label className="flex items-center gap-3"><input type="checkbox" checked={checked} onChange={(e) => onChange(e.target.checked)} className="w-4 h-4 rounded border-slate-300 text-red-600 focus:ring-red-500" /><span className="text-sm font-medium text-slate-700">{label}</span></label>;
}
