...
export function SystemAnnouncementsPage() {
  ...
  const archive = (item) => {
    if (window.confirm(`Arsipkan pengumuman “${item.title}”? Pesan akan berhenti tampil sebagai pop-up dan banner halaman utama.`)) performAction(() => archiveMutation.mutateAsync({ id: item.id }), 'Pengumuman belum dapat diarsipkan.');
  };
  const remove = (item) => {
        ...
        <div className="overflow-y-auto p-5 sm:p-6">
          ...
          <div className="grid gap-5 lg:grid-cols-2">
            ...
            <div className="space-y-4">
              ...
              <div className={`rounded-2xl border p-5 ${getSeverityStyle(form.severity).panel}`}><div className="flex items-center gap-2 text-sm font-bold"><Radio className="h-4 w-4" /> Ringkasan delivery</div><ul className="mt-3 space-y-2 text-sm leading-6 text-slate-600"><li>• Saat aktif, pengumuman tampil sebagai pop-up dan banner pada halaman utama role penerima.</li><li>• {form.requiresAcknowledgement ? 'Modal tidak dapat ditutup sebelum konfirmasi berhasil.' : 'Modal tampil satu kali per delivery version.'}</li><li>• Banner tetap tampil setelah dilihat atau dikonfirmasi hingga pengumuman berakhir atau diarsipkan.</li><li>• Target: {form.targetAllRoles ? 'semua role' : `${form.roleIds.length} role dipilih`}.</li><li>• Pengguna baru/berganti role menerima pesan selama masih aktif.</li>{publishedEdit && <li className="font-bold text-red-700">• Simpan akan menaikkan version dan mengirim ulang.</li>}</ul></div>
