...
import { AthleteTransferDetailModal } from '../components/athlete-transfers/AthleteTransferDetailModal';
import {
  transferStatusLabel,
  transferStatusStyle,
} from '../components/athlete-transfers/transferPresentation';
import {
  useAthleteTransfers,
  useAthleteTransferSummary,
...
export function AthleteTransfersPage() {
  ...
  const {
    ...
    dataUpdatedAt,
    refetch,
  } = useAthleteTransfers(filters);
  const { data: summary } = useAthleteTransferSummary();
  ...
  return (
    <DashboardLayout
      title="Transfer Atlet"
      subtitle="Kelola perpindahan atlet antar-KONI kabupaten/kota dengan alur persetujuan yang transparan."
    >
      <div className="space-y-5">
        <section className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm">
          ...
          <div className={`grid gap-3 p-4 ${box === 'inbox' ? 'sm:grid-cols-[minmax(0,1fr)_auto]' : 'sm:grid-cols-[minmax(0,1fr)_260px_auto]'}`}>
            <label className="relative block min-w-0">
              <span className="sr-only">Cari transfer atlet</span>
              <Search className="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" aria-hidden="true" />
              <input
                type="search"
                value={searchInput}
                onChange={(event) => setSearchInput(event.target.value)}
                placeholder="Cari atlet atau organisasi..."
            ...
            <div className="min-w-0">
              ...
              <div className="mt-0.5 flex flex-wrap items-center gap-1.5 text-xs text-slate-500">
                {isFetching && !isLoading ? (
                  ...
                  <span>Daftar pengajuan transfer atlet</span>
                )}
              </div>
            </div>
            <button
              ...
              <div className="hidden overflow-x-auto md:block">
                <table className="w-full min-w-[980px]">
                  <thead className="border-b border-slate-100 bg-slate-50/80 text-left text-[11px] font-bold uppercase tracking-wider text-slate-500">
                    <tr>
                      <th className="px-5 py-3.5">Atlet</th>
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between gap-3 rounded-xl bg-slate-50 px-3 py-2.5">
                      ...
                      <button
                        type="button"
                        onClick={() => setSelectedId(item.id)}
                        aria-label={`Lihat detail transfer ${item.athlete_name_snapshot || 'atlet'}`}
        ...
        {!isLoading && (!isError || items.length > 0) && total > 0 && (
          <nav
            ...
            aria-label="Paginasi transfer atlet"
          >
