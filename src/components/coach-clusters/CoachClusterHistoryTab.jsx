...
export function CoachClusterHistoryTab({ coach, readOnly = false }) {
  ...
  return (
    <div className="rounded-xl bg-slate-50 p-4">
        ...
        <div className="mb-4 rounded-lg border border-slate-100 bg-white p-3">
          ...
          {historiesQuery.isLoading ? (
            ...
            <div className="flex flex-wrap items-center gap-2">
              <span className={`rounded-lg px-2 py-1 text-xs font-semibold ${activeHistory?.is_development_cluster ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-600'}`}>{activeHistory?.cluster_label || 'Pelatih Non Binaan'}</span>
      ...
      {isMoveModalOpen && <CoachClusterMoveModal isOpen={isMoveModalOpen} onClose={() => setIsMoveModalOpen(false)} coach={coach} />}
    </div>
  );
}