...
import {
  useCoachClusterHistories,
  useCoachDevelopmentFunds,
  useDeleteCoachDevelopmentFund,
} from '../../hooks/queries/useCoachClusters';
...
export function CoachDevelopmentFundsTab({ coach, readOnly = false, onOpenClusterHistory }) {
  ...
  const fundsQuery = useCoachDevelopmentFunds(coach?.id, { year, perPage: 50 });
  const historiesQuery = useCoachClusterHistories(coach?.id);
  ...
  return (
    <div className="rounded-xl bg-slate-50 p-4">
      <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <h3 className="flex items-center gap-2 text-sm font-semibold text-slate-800"><Wallet className="h-4 w-4 text-green-600" />Biaya Pembinaan</h3>
        <div className="flex flex-wrap items-center gap-2">
          ...
          {canManage && (
            <button type="button" onClick={openCreate} disabled={statusChecking || statusUnavailable} className="inline-flex items-center gap-1.5 rounded-lg bg-red-600 px-3 py-1.5 text-xs font-semibold text-white transition-colors hover:bg-red-700 disabled:cursor-not-allowed disabled:opacity-60">
      ...
      <div className="mb-4 rounded-lg border border-green-100 bg-green-50 p-3"><p className="text-xs text-green-700">Total tahun {year}</p><p className="text-lg font-bold text-green-800">{formatCurrency(fundsQuery.data?.total_amount ?? total)}</p></div>
            ...
            <tbody className="divide-y divide-slate-100">
              {funds.map((fund) => (
                <tr key={fund.id}>
                  ...
                  {canManage && (
                    <td className="px-3 py-2"><div className="flex items-center justify-end gap-1"><button type="button" onClick={() => openEdit(fund)} disabled={deleteMutation.isPending} className="rounded-lg p-1.5 text-slate-500 transition-colors hover:bg-slate-100 hover:text-blue-600 disabled:opacity-50" title="Edit"><Edit2 className="h-3.5 w-3.5" /></button><button type="button" onClick={() => handleDelete(fund)} disabled={deleteMutation.isPending} className="rounded-lg p-1.5 text-slate-500 transition-colors hover:bg-red-50 hover:text-red-600 disabled:opacity-50" title="Hapus"><Trash2 className="h-3.5 w-3.5" /></button></div></td>
                  )}
                </tr>
              ))}
            </tbody>
      ...
      {isModalOpen && <CoachDevelopmentFundModal
        isOpen={isModalOpen}
