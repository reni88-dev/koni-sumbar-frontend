...
export function AthleteTransferHistory({ athleteId }) {
  const {
    ...
    isFetching,
    refetch,
  } = useAthleteTransferHistory(athleteId);
    ...
    <section className="space-y-4">
      <div className="flex flex-col gap-3 rounded-2xl border border-slate-200/80 bg-white p-4 shadow-xs sm:flex-row sm:items-center sm:justify-between sm:p-5">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="font-bold text-slate-800">Riwayat Transfer Atlet</h3>
      ...
      {isLoading ? (
        ...
        <HistoryState
          ...
          description="Riwayat perpindahan organisasi atlet ini akan tampil di sini."
        />
      ) : (
        <>
          {isError && (
