...
import { useAthleteDevelopmentFunds, useDeleteAthleteDevelopmentFund } from '../../hooks/queries/useAthleteClusters';
import { usePermission } from '../../hooks/usePermission';
import { AthleteDevelopmentFundModal } from './AthleteDevelopmentFundModal';

export function AthleteDevelopmentFundsTab({ athlete }) {
  const { can } = usePermission();
  const [year, setYear] = useState(String(new Date().getFullYear()));
  const [modalFund, setModalFund] = useState(null);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const { data, isLoading } = useAthleteDevelopmentFunds(athlete?.id, { year, perPage: 50 });
    ...
    <div className="p-4 bg-slate-50 rounded-xl">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {canManage && (
            <button onClick={openCreate} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-red-600 text-white rounded-lg text-xs font-semibold hover:bg-red-700 transition-colors">
      ...
      <div className="mb-4 p-3 bg-green-50 rounded-lg border border-green-100">
        ...
        <p className="text-lg font-bold text-green-800">{formatCurrency(data?.total_amount || total)}</p>
      </div>
        ...
        <div className="overflow-x-auto bg-white rounded-lg border border-slate-100">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {funds.map((fund) => (
                <tr key={fund.id}>
                  <td className="px-3 py-2 text-slate-700 whitespace-nowrap">{formatDate(fund.fund_date)}</td>
                  <td className="px-3 py-2 font-semibold text-slate-800 whitespace-nowrap">{formatCurrency(fund.amount)}</td>
                  <td className="px-3 py-2 text-slate-600 whitespace-nowrap">{fund.cluster_history?.sub_cluster_label || fund.cluster_history?.cluster_label || '-'}</td>
                  <td className="px-3 py-2 text-slate-600">{fund.description || '-'}</td>
                  {canManage && (
                    <td className="px-3 py-2">
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => openEdit(fund)} className="p-1.5 hover:bg-slate-100 rounded-lg text-slate-500 hover:text-blue-600 transition-colors" title="Edit">
                          <Edit2 className="w-3.5 h-3.5" />
                        </button>
                        <button onClick={() => handleDelete(fund)} disabled={deleteMutation.isPending} className="p-1.5 hover:bg-red-50 rounded-lg text-slate-500 hover:text-red-600 transition-colors disabled:opacity-50" title="Hapus">
      ...
      <AthleteDevelopmentFundModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} athlete={athlete} fund={modalFund} />
    </div>
  );
}
