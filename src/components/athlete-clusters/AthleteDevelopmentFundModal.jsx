...
export function AthleteDevelopmentFundModal({
  isOpen,
  onClose,
  athlete,
  fund,
  ...
  const [form, setForm] = useState(() => {
    ...
    if (fund) {
      return {
        fund_date: fund.fund_date || defaultDate,
      ...
      <Motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="fixed inset-0 z-[61] flex items-center justify-center overflow-y-auto p-4">
        <form onSubmit={handleSubmit} className="my-4 w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-xl" onClick={(event) => event.stopPropagation()}>
          <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
            ...
            <button type="button" onClick={onClose} disabled={isPending} className="rounded-lg p-2 transition-colors hover:bg-slate-100 disabled:opacity-50" aria-label="Tutup">
          ...
          <div className="max-h-[calc(100vh-12rem)] space-y-4 overflow-y-auto p-6">
            {historicalOnly && (
              <div className="rounded-xl border border-blue-100 bg-blue-50 p-3 text-sm text-blue-800">
                Mode historis aktif. Pilih tanggal pada salah satu periode Binaan sebelumnya.
              </div>
            )}
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
              ...
              {developmentPeriods.length ? (
                <ul className="list-disc space-y-1 pl-5 text-sm text-slate-700">
                  {developmentPeriods.map((period) => <li key={`${period.historyId ?? 'period'}-${period.startDate}`}>{formatDevelopmentPeriod(period)}</li>)}
                </ul>
              ) : (
            ...
            <label className="block space-y-1">
              ...
              <DateInput value={form.fund_date} onChange={(event) => setForm({ ...form, fund_date: event.target.value })} className="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm outline-none focus:border-red-500 focus:ring-2 focus:ring-red-100" />
            </label>
            <label className="block space-y-1">
              <span className="text-sm font-medium text-slate-700">Nominal</span>
              <input type="text" inputMode="numeric" value={form.amount} onChange={(event) => setForm({ ...form, amount: formatThousands(event.target.value) })} className="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm outline-none focus:border-red-500 focus:ring-2 focus:ring-red-100" placeholder="0" />
            </label>
            <label className="block space-y-1">
              <span className="text-sm font-medium text-slate-700">Keterangan</span>
              <textarea value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} rows={3} className="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm outline-none focus:border-red-500 focus:ring-2 focus:ring-red-100" />
            </label>
          </div>
          <div className="flex justify-end gap-3 border-t border-slate-100 px-6 py-4">
            ...
            <button type="submit" disabled={isPending || developmentPeriods.length === 0} className="flex items-center gap-2 rounded-xl bg-red-600 px-4 py-2.5 font-semibold text-white transition-colors hover:bg-red-700 disabled:cursor-not-allowed disabled:opacity-50">
