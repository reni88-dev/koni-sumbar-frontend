...
function MetricCard({ icon, label, value, helper, to, tone = 'text-red-600 bg-red-50' }) {
  ...
  return to ? <Link to={to} className="block h-full focus:outline-none focus:ring-2 focus:ring-red-300">{content}<span className="sr-only">Lihat rincian</span></Link> : content;
}
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <MetricCard icon={Users} label="Total profil" value={formatQualityNumber(data.profiles_total)} helper="Jumlah atlet dan pelatih sesuai filter dan hak akses Anda." />
        <MetricCard icon={TrendingUp} label="Rata-rata kualitas data" value={`${formatQualityScore(data.average_score)}%`} helper="Semakin tinggi, semakin banyak pemeriksaan data yang terpenuhi. Bukan nilai prestasi." tone="bg-blue-50 text-blue-600" />
        <MetricCard icon={FileWarning} label="Masalah belum selesai" helper="Masalah yang belum ditangani atau sedang ditangani. Satu profil dapat memiliki beberapa masalah." value={formatQualityNumber(data.active_finding_count)} tone="bg-amber-50 text-amber-600" />
        <MetricCard icon={ShieldAlert} label="Perlu segera ditangani" helper="Bagian dari masalah belum selesai yang berprioritas tinggi. Dahulukan peninjauannya." value={formatQualityNumber(data.critical_count)} tone="bg-red-50 text-red-600" />
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <h2 className="text-lg font-bold text-slate-800">Kategori kelengkapan</h2><p className="mt-1 text-sm text-slate-500">Jumlah profil dalam setiap kategori skor.</p>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <h2 className="text-lg font-bold text-slate-800">Masalah menurut tingkat prioritas</h2>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <h2 className="text-lg font-bold text-slate-800">Jenis masalah yang perlu ditangani</h2>
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="border-b border-slate-100 p-5"><h2 className="text-lg font-bold text-slate-800">Perkembangan kualitas data</h2><p className="mt-1 text-sm text-slate-500">Perbandingan hasil pemeriksaan yang tersimpan dari waktu ke waktu.</p></div>
          <div className="max-h-96 overflow-auto">
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-100">{!data.trend?.length && <tr><td colSpan={4} className="px-4 py-6 text-center text-slate-500">Belum ada riwayat pemeriksaan untuk dibandingkan.</td></tr>}{(data.trend || []).map((item, index) => <tr key={`${item.date}-${item.profile_type}-${index}`}><td className="px-4 py-3">{formatQualityDate(item.date)}</td><td className="px-4 py-3">{qualityLabel(item.profile_type)}</td><td className="px-4 py-3 text-right font-semibold">{formatQualityScore(item.average_score)}%</td><td className="px-4 py-3 text-right">{formatQualityNumber(item.profiles)}</td></tr>)}</tbody>
            </table>
          </div>
        </section>
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="border-b border-slate-100 p-5"><h2 className="text-lg font-bold text-slate-800">Peringkat kualitas data</h2><p className="mt-1 text-sm text-slate-500">Perbandingan skor antarorganisasi dan cabang olahraga, bukan peringkat prestasi.</p></div>
          <div className="max-h-96 overflow-auto divide-y divide-slate-100">
            {!data.rankings?.length && <p className="p-5 text-sm text-slate-500">Belum ada data peringkat untuk filter ini.</p>}{(data.rankings || []).map((item, index) => {
      ...
      <div className="relative h-full w-full max-w-xl overflow-y-auto bg-white p-5 shadow-2xl sm:p-7">
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2">
          {[
            ...
            ['NIK (disamarkan)', item.nik_display], ['No. KK (disamarkan)', item.kk_display], ['Telepon (disamarkan)', item.phone_display], ['Email (disamarkan)', item.email_display],
...
export function DataQualityReportContent({ reportKey, response, filters, allowedReportKeys, readOnly = false }) {
  ...
  switch (reportKey) {
    ...
    case 'athletes':
    case 'coaches':
      return <ProfileReport rows={rows} readOnly={readOnly} />;
    case 'duplicates':
