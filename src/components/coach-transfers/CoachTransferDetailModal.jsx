...
import {
  fetchCoachTransferDocument,
  useCoachTransfer,
  useCancelCoachTransfer,
  useDestinationReviewCoachTransfer,
  useKoniReviewCoachTransfer,
} from '../../hooks/queries/useCoachTransfers';
import { transferStatusLabel, transferStatusStyle } from './transferPresentation';

export function CoachTransferDetailModal({ id, isOpen, onClose }) {
  return (
    <AnimatePresence>
      {isOpen && id && (
        <CoachTransferDetailDialog key={id} id={id} onClose={onClose} />
      )}
    </AnimatePresence>
  );
}

function CoachTransferDetailDialog({ id, onClose }) {
  const { user } = useAuth();
  ...
  const {
    ...
    isFetching,
    refetch,
  } = useCoachTransfer(id, true);
  const destinationReview = useDestinationReviewCoachTransfer();
  const koniReview = useKoniReviewCoachTransfer();
  const cancelTransfer = useCancelCoachTransfer();
  ...
  const titleId = `coach-transfer-detail-title-${id}`;
  ...
  const openDocument = async (mode) => {
    ...
    try {
      const blob = await fetchCoachTransferDocument(id);
  ...
  const submitReview = async () => {
    ...
    try {
      ...
      setActionError(getApiMessage(
        error,
        blockers.length
          ? 'Transfer belum dapat diselesaikan karena pelatih masih memiliki penugasan aktif.'
        ...
        <header className="relative shrink-0 border-b border-slate-100 bg-white px-5 py-4 sm:px-6 sm:py-5">
          <div className="flex min-w-0 flex-col gap-2 pr-12 sm:flex-row sm:items-center sm:gap-3">
            <div className="min-w-0">
              <p className="text-xs font-bold uppercase tracking-wider text-red-600">Transfer Pelatih</p>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="mt-0.5 text-sm text-amber-800/80">
                        Tindakan akan dicatat pada timeline transfer pelatih.
                      </p>
                    </div>
                  </div>
                          ...
                          {actionBlockers.map((blocker) => (
                            <li key={`${blocker.kind}-${blocker.event_id}`} className="rounded-lg bg-white/80 px-3 py-2">
                              <p className="font-bold text-red-800">
                                {coachTransferBlockerLabel(blocker.kind)}: {blocker.event_name || 'Penugasan aktif'}
                              </p>
      ...
      <div className="relative">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="text-xs font-bold uppercase tracking-wider text-slate-400">Pelatih</p>
...
function OutcomeMessage({ item }) {
  ...
  if (item.status === 'completed') {
    ...
    message = 'Organisasi pelatih telah diperbarui ke organisasi tujuan.';
...
function coachTransferBlockerLabel(kind) {
  return ({
    assignment: 'Atlet binaan aktif',
    training_schedule: 'Jadwal latihan aktif',
    training_session: 'Sesi latihan aktif',
    porprov: 'Penugasan Porprov aktif',
    porprov_registration: 'Pendaftaran Porprov aktif',
  }[kind] || 'Penugasan aktif');
}
