...
import {
  useCoachTransferDestinations,
  useCreateCoachTransfer,
...
export function CoachTransferCreateModal({ coach, isOpen, onClose, onSuccess }) {
  return (
    <AnimatePresence>
      {isOpen && coach && (
        <CoachTransferCreateDialog
          key={coach.id}
          coach={coach}
          onClose={onClose}
          onSuccess={onSuccess}
...
function CoachTransferCreateDialog({ coach, onClose, onSuccess }) {
  ...
  } = useCoachTransferDestinations(coach.id, true);
  const mutation = useCreateCoachTransfer();
  const maxDate = todayJakarta();
  const titleId = `coach-transfer-create-title-${coach.id}`;
  const descriptionId = `coach-transfer-create-description-${coach.id}`;
  ...
  const organizationName = coach.organization?.name
    || coach.organization_name
    || '-';
  const caborName = coach.cabor?.display_name
    || coach.cabor?.name
    || coach.cabor_name
    || '-';
        ...
        <div className="flex-1 space-y-5 overflow-y-auto bg-slate-50/40 p-4 sm:p-6">
          <section className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-xs">
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              <SummaryItem icon={UserRound} label="Pelatih" value={coach.name} />
          ...
          <section className="space-y-4 rounded-2xl border border-slate-200/80 bg-white p-4 shadow-xs sm:p-5">
            ...
            <div className="space-y-1.5">
              ...
              {!destinationsLoading && !destinationsError && destinations.length === 0 && (
                <p className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800">
                  Tidak ada organisasi tujuan yang tersedia untuk pelatih ini.
                </p>
              )}
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <textarea
                ...
                disabled={mutation.isPending}
                placeholder="Jelaskan alasan perpindahan pelatih secara ringkas dan jelas..."
